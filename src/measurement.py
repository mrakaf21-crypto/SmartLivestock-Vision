...
MODULE: measurement.py (ADAPTIVE EDITION - NO EMOJI CLEAN VERSION)
============================================================
"""
...
def load_config():
    if not os.path.exists(_CONFIG_PATH):
        return {
            "mm_per_pixel": 0.27, 
            "scale_length": 48.3871, 
            "scale_height": 63.1579
        }
...
def toy_to_real_length(toy_mm: float) -> float:
    cfg = load_config()
    ratio = cfg.get("scale_length", 48.3871) / 10.0
    return toy_mm * ratio

def toy_to_real_height(toy_mm: float) -> float:
    cfg = load_config()
    ratio = cfg.get("scale_height", 63.1579) / 10.0
    return toy_mm * ratio
...
# --- Keseragaman (Robust Stats with Error Handling) ---

def calculate_uniformity(weights: list) -> dict:
    if not weights or len(weights) < 2:
        return {
            "mean": 0, "std": 0, "cv": 0,
            "uniformity": 0, "status": "Butuh minimal 2 sapi untuk kalkulasi kelompok.",
